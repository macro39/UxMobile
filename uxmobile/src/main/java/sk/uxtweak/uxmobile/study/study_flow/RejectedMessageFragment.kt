...
class RejectedMessageFragment: Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val rejectedMessage: StudyMessage = (activity as StudyFlowFragment).getData(this) as StudyMessage

        textView_rejected_message_title.text = rejectedMessage.title

        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            textVIew_rejected_message_consent.text = Html.fromHtml(rejectedMessage.content, Html.FROM_HTML_MODE_COMPACT)
        } else {
