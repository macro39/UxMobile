...
class WelcomeMessageFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val welcomeMessage : StudyMessage = (activity as StudyFlowFragment).getData(this) as StudyMessage

        textView_welcome_message_title.text = welcomeMessage.title

        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            textVIew_welcome_message_consent.text = Html.fromHtml(welcomeMessage.content, Html.FROM_HTML_MODE_COMPACT)
        } else {
